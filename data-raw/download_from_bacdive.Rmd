...
if (file.exists("data-raw/type_strain_large_list.rda")) {
  ...
  full_list <-
    read_csv(
      "data-raw/full_list_of_bacteria_from_bacdive_20211027.csv",
      skip = 2,
...
for (i in 1:length(list_holder)) {
  ...
  if (is.null(list_holder[[i]]$Morphology$`cell morphology`$`gram stain`)) {
    ...
    bacdive_phenotypes[i, "gram_stain"] <-
      list_holder[[i]]$Morphology$`cell morphology`$`gram stain`
  }
  if (is.null(list_holder[[i]]$`Physiology and metabolism`$`oxygen tolerance`$`oxygen tolerance`)) {
    ...
    bacdive_phenotypes[i, "aerobic_status"] <-
      list_holder[[i]]$`Physiology and metabolism`$`oxygen tolerance`$`oxygen tolerance`
    # TODO this breaks when you have multiple references for the aerobic status
    # what we really need here is sth like bind_rows(list_holder[[i]]$`Physiology and metabolism`$`oxygen tolerance`)
    # and then taking them all and doing the filtering below
    # might as well do this for gram stain too and then join the tables
  }
}
...
bacdive_phenotypes %<>%
  filter(!is.na(gram_stain) | !is.na(aerobic_status))

# there are some with multiple entries
# let's take the ones that are most frequent
dupes <- bacdive_phenotypes %>% count(taxon) %>% filter(n > 1) %>%
  left_join(bacdive_phenotypes, by = "taxon") %>%
  group_by(taxon) %>%
  count(gram_stain, aerobic_status) %>%
  filter(n == max(n)) %>% #gets the most common, this would not work if you had a tie
  ungroup()

bacdive_phenotypes %<>%
  group_by(taxon) %>%
  slice(1) %>% #remove any duplicates
  mutate(
    gram_stain = ifelse(taxon %in% dupes$taxon, dupes$gram_stain, gram_stain),
    #if they were a duplicate, replace with most common information
    aerobic_status = ifelse(taxon %in% dupes$taxon, dupes$aerobic_status, aerobic_status)
  )
